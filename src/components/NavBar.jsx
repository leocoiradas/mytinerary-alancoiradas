    ...
    return (
        <nav className={`${isOpen ? 'block text-center' : 'hidden'} md:flex md:items-center md:w-auto`}>
            <ul className={`flex flex-col md:flex-row md:gap-4  ${isOpen ? 'block gap-2 md:flex' : 'hidden'} md:flex w-full`}>
                {links.map((link) => (<li key={link.title}><Anchor className={`p-1.5 rounded-lg text-lg md:p-3 md:text-base hover: ${link.isUser ? ' bg-blue-700 text-zinc-100' : ''}`} to={link.to}>{link.title}</Anchor></li>))}
            </ul>
        </nav>
    );
};
